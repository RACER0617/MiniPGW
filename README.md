...
- Получает и отображает ответ сервера (например, "created", "rejected" или "refresh").
...
```bash
./src/server/pgw_server ../config_server.json
```

- **`../config_server.json`**: Путь к файлу конфигурации сервера. Если нужен режим debug, необходимо это указать в конфиге серфера (`log_level`).
...
```bash
./src/client/pgw_client ../config_client.json <IMSI>
```

- **`../config_client.json`**: Путь к файлу конфигурации клиента. Если нужен режим debug, необходимо это указать в конфиге серфера (`log_level`).
...
Скрипт `script_hardtest.sh` предназначен для нагрузочного тестирования клиента `pgw_client`. Он отправляет 5000 (можете поменять на большее число) случайных IMSI-запросов на сервер асинхронно, чтобы проверить производительность и устойчивость системы.
...
1. Убедитесь, что сервер запущен:
   ...
   ./src/server/pgw_server ../config_server.json
   ```
2. Убедитесь, что файл `config_client.json` находится в корневой директории проекта.
3. Сделайте скрипт исполняемым:
   ```bash
   chmod +x script_hardtest.sh
   ```
4. Запустите скрипт из корневой директории проекта:
   ```bash
   ./script_hardtest.sh
   ```

#### Описание скрипта
- **Назначение**: Скрипт генерирует 5000 случайных 15-значных IMSI и отправляет их на сервер с помощью `pgw_client` в фоновом режиме, используя конфигурацию из `config_client.json`.
- **Вывод**: Для каждого IMSI отображается сообщение о его отправке. После завершения всех запросов выводится сообщение "All 5000 random IMSI requests sent."
- **Файл**: `script_hardtest.sh` находится в корневой директории проекта.
...
- **Журнал CDR**: События сессий (создание, удаление, завершение) записываются в `cdr.log` (настраивается в `config_server.json`).

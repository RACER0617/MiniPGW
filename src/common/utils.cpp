...
std::vector<uint8_t> imsiStringToBcd(const std::string& imsi) {
    if (imsi.length() != 15) throw std::invalid_argument("IMSI must be 15 digits");
    std::vector<uint8_t> bcd(8, 0);
    for (size_t i = 0; i < 15; ++i) {
        char c = imsi[i];
        if (!isdigit(static_cast<unsigned char>(c))) throw std::invalid_argument("IMSI must contain only digits");
        int digit = c - '0';
        if (i % 2 == 0) {
...
std::string bcdToImsiString(const std::vector<uint8_t>& bcd) {
    if (bcd.size() != 8) throw std::invalid_argument("BCD must be 8 bytes");
    std::string imsi;
